<template>
    <section class="bg-white dark:bg-[#111C44] rounded-2xl">
        ...
        <AppTable :header="cashierTableHeader" :fields="selectedCashier" :loading="loading2" 
               :empty="error">
...
const getCashierDetails = async() => {
    try{
        loading2.value = true;
        const res = await axios.get(`report/customerterminal/dailygame?fromDate=${startDate.value}&toDate=${endDate.value}&shopCode=${authStore.user.shopCode}&CashierId=${cashierId.value}`)
